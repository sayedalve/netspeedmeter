...
// ═════════════════════════════════════════════════════════════════════════════
//  Design tokens  — Red & Black theme, Windows 11 flat-card aesthetic
// ═════════════════════════════════════════════════════════════════════════════
namespace Theme {

// Palette
constexpr const char* BG_WINDOW    = "#1A1A1A";   // dialog background
constexpr const char* BG_CARD      = "#242424";   // group box / card
constexpr const char* BG_INPUT     = "#2E2E2E";   // spinbox / combo bg
constexpr const char* BG_HOVER     = "#383838";   // hover state
constexpr const char* ACCENT       = "#E53935";   // red accent
...
constexpr const char* TAB_SEL_BG   = "#2E2E2E";

// Sizes
constexpr int RADIUS        = 6;
constexpr int CARD_RADIUS   = 8;
constexpr int SPACING       = 10;
constexpr int LABEL_W       = 160;  // left-column label width in form layouts
...
ColorButton::ColorButton(const QString& initialHex, QWidget* parent)
    : QPushButton(parent)
    , m_color(initialHex)
{
    setFixedSize(44, 28);
    setCursor(Qt::PointingHandCursor);
    setToolTip(tr("Click to choose colour"));
    applyColor();
...
void ColorButton::onClicked()
{
    const QColor chosen = QColorDialog::getColor(
        m_color, this, tr("Choose Colour"),
        QColorDialog::ShowAlphaChannel);

    if (chosen.isValid()) {
        m_color = chosen;
        applyColor();
        emit colorChanged(colorHex());
    }
}

void ColorButton::applyColor()
{
    // Paint the button face with the colour swatch
    const QString bg = m_color.name(QColor::HexArgb);
    setStyleSheet(QStringLiteral(
        "QPushButton { background-color: %1; border: 2px solid #5A5A5A; border-radius: 5px; }"
        "QPushButton:hover { border-color: #E53935; }"
    ).arg(bg));
}

QString ColorButton::colorHex() const
{
    // Return #RRGGBB (no alpha) for non-alpha colours, or ARGB for those with
    return m_color.name(m_color.alpha() < 255 ? QColor::HexArgb : QColor::HexRgb);
}

void ColorButton::setColorHex(const QString& hex)
{
    m_color = QColor(hex);
    applyColor();
}
...
// ── Helper: labelled row for form layouts ─────────────────────────────────────
...
static QFrame* makeSeparator()
{
    ...
    f->setStyleSheet(QStringLiteral("color: #303030;"));
    return f;
}
...
{
    // Wrap in a scroll area so it works at low DPI / small screens
    ...
    m_opacitySlider->setRange(5, 100);       // 5% – 100%
    ...
    m_fontScaleSlider->setRange(50, 300);    // 0.50× – 3.00×
    ...
    auto* fontLbl = new QLabel(tr("Font scale"), fontGroup);
    fontLbl->setFixedWidth(Theme::LABEL_W);
    fontLbl->setAlignment(Qt::AlignRight | Qt::AlignVCenter);
    fontLbl->setStyleSheet(QStringLiteral("color: #C0C0C0; font-size: 13px;"));

    fontRow->addWidget(fontLbl);
    fontRow->addWidget(m_fontScaleSlider, 1);
    fontRow->addWidget(m_fontScaleLabel);
    fontVBox->addLayout(fontRow);
    ...
    // ── Graph ─────────────────────────────────────────────────────────────────
    auto* graphGroup = new QGroupBox(tr("Mini-Graph"), inner);
    auto* graphVBox  = new QVBoxLayout(graphGroup);
    graphVBox->setSpacing(10);

    m_showGraphCheck = new QCheckBox(tr("Show area-chart background graph"), graphGroup);
    graphVBox->addWidget(m_showGraphCheck);

    m_graphHistorySpin = new QSpinBox(graphGroup);
    m_graphHistorySpin->setRange(10, 300);
    m_graphHistorySpin->setSuffix(tr(" samples"));
    m_graphHistorySpin->setToolTip(tr("How many data points the graph stores (10–300)"));
    graphVBox->addLayout(labeledRow(tr("History size"), m_graphHistorySpin));

    vbox->addWidget(graphGroup);
    ...
    m_speedUnitCombo->addItem(tr("Auto  (B / KB / MB / GB)"),  QStringLiteral("auto"));
    m_speedUnitCombo->addItem(tr("Always KB/s"),               QStringLiteral("kbps"));
    m_speedUnitCombo->addItem(tr("Always Mbps"),               QStringLiteral("mbps"));
    displayVBox->addLayout(labeledRow(tr("Unit"), m_speedUnitCombo));
    ...
    // ── Colours ───────────────────────────────────────────────────────────────
    auto* colorGroup = new QGroupBox(tr("Colours"), inner);
    auto* colorVBox  = new QVBoxLayout(colorGroup);
    colorVBox->setSpacing(10);

    m_accentColorBtn = new ColorButton(QStringLiteral("#E53935"), colorGroup);
    m_bgColorBtn     = new ColorButton(QStringLiteral("#CC000000"), colorGroup);
    m_textColorBtn   = new ColorButton(QStringLiteral("#FFFFFF"), colorGroup);

    colorVBox->addLayout(labeledRow(tr("Accent colour"),      m_accentColorBtn));
    colorVBox->addLayout(labeledRow(tr("Background colour"),  m_bgColorBtn));
    colorVBox->addLayout(labeledRow(tr("Text colour"),        m_textColorBtn));

    auto* colorHint = new QLabel(
        tr("Click a swatch to open the colour picker. Background supports alpha."),
        colorGroup);
    colorHint->setWordWrap(true);
    colorHint->setStyleSheet(QStringLiteral("color: #666666; font-size: 11px;"));
    colorVBox->addWidget(colorHint);
...
void SettingsDialog::populateAdapterList()
{
    ...
    for (const AdapterInfo& a : adapters) {
        ...
        item->setData(Qt::UserRole, a.guid);   // store GUID for collection
        item->setToolTip(a.guid);
    }

    // Update enabled state based on current mode
...
void SettingsDialog::loadFromConfig(const AppConfig& cfg)
{
    ...
    // Adapter list checkboxes are set via populateAdapterList() using
    // the config.selectedAdapterGuids — already called in constructor.
    ...
    if (m_accentColorBtn)  m_accentColorBtn->setColorHex(cfg.accentColor);
    if (m_bgColorBtn)      m_bgColorBtn->setColorHex(cfg.backgroundColor);
    if (m_textColorBtn)    m_textColorBtn->setColorHex(cfg.textColor);
}

AppConfig SettingsDialog::collectToConfig() const
{
    // Start from the current persisted config so any fields not represented
    // in the dialog (e.g. widgetPos) are preserved.
    ...
    if (m_accentColorBtn)   cfg.accentColor    = m_accentColorBtn->colorHex();
    if (m_bgColorBtn)       cfg.backgroundColor = m_bgColorBtn->colorHex();
    if (m_textColorBtn)     cfg.textColor      = m_textColorBtn->colorHex();
...
void SettingsDialog::onReset()
{
    ...
    loadFromConfig(AppConfig{});   // load default-constructed config
}
...
// ─────────────────────────────────────────────────────────────────────────────
//  Unused helper declared in header — kept for potential Batch 5 use
// ─────────────────────────────────────────────────────────────────────────────
